...
cd ${WORKDIR}
python ${SRCDIR}/cjs-ledger ${DATADIR} --exec npm --cache=./cache install @nested/example@0.1.0

test -d ${WORKDIR}/node_modules/@nested/example || die "Package not installed"
